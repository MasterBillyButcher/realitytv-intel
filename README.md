...
cd realitytv-intel
npx serve .        # or any static server — opening index.html directly via file:// won't work,
                    # since the app fetches /api/data, data/data.js, and vendor/ scripts by relative path
```

Or just visit **[realitytv-intel.vercel.app](https://realitytv-intel.vercel.app/)** — no setup needed to browse. Note the `/api/*` routes (live database, Instagram refresh, admin login) only run on Vercel — a local static server shows the site and dashboard fine, but those specific features need an actual `vercel dev` or deployed environment.
...
3. **🚀 Publish Live** → writes straight to the database — everyone sees it within ~20s, no upload step, no redeploy

**One-time setup, before your first publish:** Vercel dashboard → your project → **Storage** tab → create a Redis/KV database → connect it to this project → redeploy once. Then open **⚙ Shows** and click **⬆ Migrate current data to database** — one click, copies everything currently loaded into the live store. After that, **🚀 Publish Live** is the only step you'll ever need again.

Want an offline copy for your own records? **↓ Download Backup** in the Export panel still works exactly like the old "Save JSON" did — it just isn't the publish mechanism anymore.
...
Open <b>⚙ Shows</b> in the admin panel — it shows live database status at the top. If it says "serving the bundled backup file," the database isn't connected yet or hasn't been migrated (see the one-time setup note above). If it says "Connected," changes should appear for other visitors within ~20s of clicking <b>🚀 Publish Live</b> — check your browser console for a failed <code>/api/data</code> request if it's still stale beyond that.
</details>
